...
from connect4_env import Connect4Env, CustomConnect4CNN  # Import CustomConnect4CNN from connect4_env
...
def train_self_play(total_timesteps=1000000, update_interval=2000, eval_threshold=0.55, max_hours=12, model_path="connect4_cnn_master"):
    num_envs = 6
    ...
    if os.path.exists(f"{model_path}.zip"):
        print(f"Attempting to load model from {model_path}.zip")
        try:
            model = PPO.load(model_path, env=env, device="cuda")
            model.learning_rate = 1e-4
            print("Model loaded successfully.")
        except ValueError as e:
            print(f"Error loading model due to observation space mismatch: {e}")
        ...
        print("Starting new CNN model training")
        model = PPO(
            "CnnPolicy",
            env,
            verbose=1,
    ...
    while timesteps_so_far < total_timesteps and (time.time() - start_time) < max_seconds:
        if timesteps_so_far < 100000:
            env.env_method('set_opponent_params', None)
            print(f"Training against random opponent at {timesteps_so_far} timesteps")
        elif timesteps_so_far < 300000:
